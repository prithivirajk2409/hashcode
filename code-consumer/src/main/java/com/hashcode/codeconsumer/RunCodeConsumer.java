...
public class RunCodeConsumer {

	private static final Logger logger = LogManager.getLogger(RunCodeConsumer.class);

	public static void setLog4j2Config() {
		try {
			String loggerFileName = Constants.APP_MODE.equals(AppMode.PRODUCTION)
					? Constants.FILENAME_LOG_PROD_PROPERTIES
					: Constants.FILENAME_LOG_NON_PROD_PROPERTIES;
			URL loggerPath = RunCodeConsumer.class.getClassLoader().getResource(loggerFileName);
	...
	public static void main(String args[]) {
		...
		SpringApplication.run(RunCodeConsumer.class, args);
	}
